...
- Final PDF hanya boleh memakai frozen snapshot yang tersimpan di `agreements.event_snapshot`, `agreements.party_snapshot`, `agreements.bank_snapshot`, `agreements.document_snapshot`, dan `agreements.commercial_snapshot`.
- Data PIHAK PERTAMA nantinya harus berasal dari database/admin settings pada V2-1, bukan hardcode.
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Cover | STATIC + DATABASE/SNAPSHOT | Menampilkan identitas ringkas dokumen perjanjian. | Judul dokumen, nomor dokumen, nama event, nama penyelenggara, versi template. | `agreements.document_number`, `agreements.template_version`, `agreements.event_snapshot.event_name`, `agreements.party_snapshot.organizer_name` | Final PDF wajib dari snapshot; judul hukum tetap statis. |
| 2 | Pembukaan Perjanjian | STATIC + DATABASE/SNAPSHOT | Menyatakan para pihak, konteks kerja sama, dan referensi dokumen. | Nomor dokumen, nama PIHAK PERTAMA, nama PIHAK KEDUA, referensi event. | `agreements.document_number`, source PIHAK PERTAMA V2-1, `agreements.party_snapshot.organizer_name`, `agreements.event_snapshot.event_name` | Tanggal efektif/perumusan pembukaan jangan diasumsikan bila belum diputuskan legal. |
| 3 | Identitas PIHAK PERTAMA / Gotik | CONFIG + SNAPSHOT | Menetapkan identitas hukum platform. | Nama badan usaha/pengelola, legalitas/NIB, alamat, wakil, jabatan, email, telepon/WhatsApp, situs/platform. | BELUM ADA authoritative source saat ini; target V2-1 admin settings/database. | Semua field PIHAK PERTAMA saat ini masih gap. |
...
| 6 | PASAL 2 - Maksud, Tujuan dan Ruang Lingkup | STATIC + SNAPSHOT | Menjelaskan kerja sama penyelenggaraan dan penjualan tiket event tertentu. | Nama event, peran platform, ruang lingkup layanan. | `agreements.event_snapshot.event_name`, data PIHAK PERTAMA V2-1 | Isi hukum statis, event menjadi referensi dinamis. |
...
| 9 | PASAL 5 - Biaya, Pajak dan Ketentuan Komersial | STATIC + SNAPSHOT | Menetapkan buyer/platform fee dan fee payment gateway yang relevan. | Buyer/platform fee, payment gateway aktif, fixed fee, percent fee, payment OTP bila relevan. | `agreements.commercial_snapshot.*` | Ini area contractual utama. |
...
| 12 | PASAL 8 - Hak dan Kewajiban PIHAK PERTAMA | STATIC + CONFIG | Menetapkan kewajiban dan hak platform. | Identitas PIHAK PERTAMA jika disebut. | Source PIHAK PERTAMA V2-1 | Jangan hardcode identitas sekarang. |
...
| 25 | PASAL 21 - Penutup | STATIC + SNAPSHOT | Menutup dokumen dan menegaskan keberlakuan perjanjian. | Nama para pihak, nomor dokumen. | `agreements.document_number`, `agreements.party_snapshot.organizer_name`, source PIHAK PERTAMA V2-1 | Teks hukum statis, identitas dinamis. |
| 26 | Halaman Penandatanganan | CONFIG + SNAPSHOT | Menyediakan blok tanda tangan para pihak. | Nama badan usaha, wakil, jabatan PIHAK PERTAMA; nama penyelenggara, penanggung jawab, jabatan PIHAK KEDUA. | PIHAK PERTAMA V2-1; `agreements.party_snapshot.*` | PIHAK PERTAMA belum punya source authoritative saat ini. |
| 27 | Lampiran I - Data Event & Ketentuan Komersial | SNAPSHOT | Merangkum data event dan komersial yang bersifat contractual. | Nama event, penyelenggara, tanggal/waktu event, venue, alamat venue, periode penjualan, buyer/platform fee, payment method, fee payment, rekening pencairan. | `agreements.event_snapshot.*`, `agreements.party_snapshot.organizer_name`, `agreements.commercial_snapshot.*`, `agreements.bank_snapshot.*` | Jangan memasukkan data tiket/kategori/harga/kuota. |
| 28 | Lampiran II - Dokumen/Kesiapan & Kontak Resmi | SNAPSHOT + CONFIG | Merangkum data dokumen pendukung dan kontak resmi yang benar-benar tersedia. | PIC penyelenggara, rekening/buku rekening, surat penyelenggara, kontak resmi para pihak, status verifikasi relevan bila ditampilkan. | `agreements.party_snapshot.*`, `agreements.bank_snapshot.*`, `agreements.document_snapshot.*`, PIHAK PERTAMA V2-1 | Jika checklist tertentu belum dikelola sistem, tulis `BELUM TERSEDIA - jangan diasumsikan`. |

## Field Source Matrix

| Bagian | Field | Jenis | Source/model/field | Sudah tersedia? | Masuk contractual diff? | Catatan |
| --- | --- | --- | --- | --- | --- | --- |
| PIHAK PERTAMA | nama badan usaha/pengelola | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1: admin settings/database -> final snapshot `agreements.platform_party_snapshot.name` atau ekuivalen | Belum | Ya, target V2-1 | Saat ini tidak ada source database yang sah; jangan hardcode. |
| PIHAK PERTAMA | legalitas/NIB | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Harus berasal dari profil legal platform resmi. |
| PIHAK PERTAMA | alamat | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Jangan ambil dari footer website atau teks bebas. |
| PIHAK PERTAMA | pemilik/wakil/penanggung jawab aplikasi | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Dibutuhkan untuk identitas dan halaman tanda tangan. |
| PIHAK PERTAMA | jabatan | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Belum ada field resmi saat ini. |
| PIHAK PERTAMA | email | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Email seed/admin sekarang bukan source kontraktual. |
| PIHAK PERTAMA | telepon/WhatsApp | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Wajib ditandai gap. |
| PIHAK PERTAMA | situs/platform | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Domain/platform name harus berasal dari setting resmi. |
...
| REKENING | buku rekening | DATABASE -> SNAPSHOT | Preview live tidak diringkas khusus; final source file metadata: `event_bank_accounts.bank_book_original_name`, `bank_book_path`, `bank_book_mime` | Sebagian | Tidak | File tersedia di data sumber, tetapi belum masuk snapshot final saat ini; jika dicantumkan di Lampiran II, implementasi v2 perlu keputusan eksplisit. |
...
| DOKUMEN | nomor dokumen agreement | DATABASE | `agreements.document_number` | Ya | Belum tercermin pada addendum | Ini nomor dokumen MOU/Addendum, bukan nomor surat penyelenggara. |
| KOMERSIAL | buyer/platform fee | DATABASE -> SNAPSHOT | Sumber dasar: `events.fee` via `TicketPricingService::tax($event, 0)`; Final: `agreements.event_snapshot.buyer_fee` dan `agreements.commercial_snapshot.buyer_fee` | Ya | Ya | Semantik saat ini: `percent`, `fixed`, atau `none`. |
...
| KOMERSIAL | fee fixed gateway | DATABASE -> SNAPSHOT | Sumber dasar: `event_payment_gateways.fee_fixed` atau fallback `payment_gateways.default_fee_fixed` / `payment_gateways.biaya`; Final: `agreements.commercial_snapshot.payment_gateways[*].resolved_fee_fixed` | Ya | Ya | Nilai final sudah dinormalisasi ke string decimal. |
| KOMERSIAL | fee percent gateway | DATABASE -> SNAPSHOT | Sumber dasar: `event_payment_gateways.fee_percent` atau fallback `payment_gateways.default_fee_percent` / `payment_gateways.biaya`; Final: `agreements.commercial_snapshot.payment_gateways[*].resolved_fee_percent` | Ya | Ya | Nilai final sudah dinormalisasi ke string decimal. |
...
| LAMPIRAN I | buyer/platform fee | SNAPSHOT | `agreements.commercial_snapshot.buyer_fee` | Ya | Ya | Termasuk data contractual. |
...
| LAMPIRAN I | fee payment | SNAPSHOT | `agreements.commercial_snapshot.payment_gateways[*].resolved_fee_fixed`, `resolved_fee_percent` | Ya | Ya | Termasuk data contractual. |
...
| LAMPIRAN II | rekening/buku rekening | SNAPSHOT + DATABASE | Snapshot: `agreements.bank_snapshot.*`; tambahan file buku rekening dari source live `event_bank_accounts.bank_book_*` | Sebagian | Rekening: Ya, buku rekening file: Tidak | Jika bukti file ditampilkan, source file ada tetapi belum dibekukan ke snapshot saat ini. |
...
| LAMPIRAN II | kontak resmi PIHAK PERTAMA | CONFIG | BELUM ADA SOURCE AUTHORITATIVE. Target V2-1 admin settings/database | Belum | Ya, target V2-1 | Tulis `BELUM TERSEDIA - jangan diasumsikan` sampai V2-1 ada. |
...
- V2-1 perlu membuat source authoritative untuk seluruh identitas PIHAK PERTAMA melalui database/admin settings.
...
- Jika Lampiran II ingin menampilkan bukti file buku rekening, perlu keputusan implementasi apakah metadata file ikut dibekukan ke snapshot final; saat ini belum ada field snapshot khusus untuk itu.
