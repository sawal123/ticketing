<div>
    ...
    <!-- Tab Navigation -->
    <div class="flex items-center gap-1 bg-white dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 w-fit mb-6">
                    ...
                    <x-admin.card title="Informasi Event">
                        <div class="space-y-4">
                            <div class="aspect-video rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-700">
                                <img src="{{ asset('storage/cover/' . $event->cover) }}" alt="{{ $event->event }}" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/800x450?text=No+Cover'">
                            </div>
                            <div>
                                <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-2">Deskripsi</h3>
                                <div class="text-slate-600 dark:text-slate-400 leading-relaxed prose dark:prose-invert max-w-none">
                                    @if($showFullDescription)
                                        {!! $event->deskripsi !!}
                                        <button wire:click="toggleDescription" class="text-indigo-600 hover:text-indigo-700 font-semibold text-sm mt-2 flex items-center gap-1">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            @forelse($event->talents as $talent)
                                <div class="flex flex-col items-center text-center group">
                                    <div class="w-20 h-20 rounded-full overflow-hidden mb-2 ring-2 ring-slate-100 dark:ring-slate-700 group-hover:ring-indigo-500 transition-all">
                                        <img src="{{ asset('storage/talent/' . $talent->gambar) }}" alt="{{ $talent->talent }}" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/150x150?text=User'">
                                    </div>
                                    <span class="text-sm font-semibold text-slate-800 dark:text-white">{{ $talent->talent }}</span>
                                </div>
                            @empty
                        ...
                        <div class="space-y-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-slate-800 dark:text-white">{{ \Carbon\Carbon::parse($event->tanggal)->format('d F Y') }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-3">
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <span class="text-sm font-semibold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300">
                ...
                @forelse($event->hargas as $harga)
                    <tr class="table-row-hover transition-colors">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex flex-col">
                                ...
                                <div class="w-20 h-1.5 bg-slate-100 rounded-full mt-1 overflow-hidden">
                                    <div class="h-full bg-emerald-500" style="width: {{ min(100, ($harga->sold_count / max(1, $harga->qty)) * 100) }}%"></div>
                                </div>
                            </div>
                        </td>
                        <td class="px-5 py-4">
                        ...
                        <td class="px-5 py-4">
                            <!-- Toggle Status -->
                            <button 
                                wire:click="toggleTicketStatus({{ $harga->id }})"
                            ...
                            >
                                <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform {{ $harga->status === 'active' ? 'translate-x-6' : 'translate-x-1' }}"></span>
                            </button>
                        </td>
                        <td class="px-5 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <x-admin.button 
                                    wire:click="editTicket({{ $harga->id }})" 
                                    variant="ghost" 
                                    size="sm" 
                                    icon="pencil"
                                ...
                                />
                                <x-admin.button 
                                    wire:click="confirmDeleteTicket({{ $harga->id }})" 
                                    variant="ghost" 
                                    size="sm" 
                                    icon="trash-2"
                                    class="text-rose-600"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ number_format($metrics['total_tickets']) }}</p>
                    </div>
                </div>
                <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">Total Omset</p>
                        <p class="text-2xl font-extrabold text-slate-800 dark:text-white">Rp {{ number_format($metrics['total_revenue'], 0, ',', '.') }}</p>
                    </div>
                </div>
                <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-extrabold text-slate-800 dark:text-white">{{ number_format($metrics['total_transactions']) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <x-admin.card padding="p-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                        <div>
                            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Metode Bayar</label>
                            <select wire:model.live="filterPayment" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 transition-all">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="flex-1">
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Cari Transaksi</label>
                                <x-admin.input 
                                    wire:model.live.debounce.300ms="searchTransaction" 
                                    placeholder="Invoice / Nama..." 
                                    icon="search" 
                                />
                            </div>
                            <x-admin.button variant="ghost" icon="rotate-ccw" wire:click="resetFilters" class="mb-0.5" title="Reset Filter"></x-admin.button>
                        </div>
                    </div>
                </x-admin.card>
                ...
                <x-admin.table title="Daftar Transaksi Terfilter" :headers="['User', 'Invoice', 'Payment', 'Tanggal', 'Aksi']" :count="$transactions->total()">
                    @forelse($transactions as $trx)
                        <tr class="table-row-hover transition-colors">
                            ...
                            <td class="px-5 py-4">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 border-indigo-200 dark:border-indigo-700">
    ...
    <x-admin.modal name="edit-ticket-modal" title="Edit Kategori Tiket" icon="pencil">
        <form wire:submit="updateTicket" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Stok (Qty)</label>
                    <x-admin.input type="number" wire:model="editingHarga.qty" required />
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Harga (Rp)</label>
            ...
            <div>
                ...
                <select wire:model="editingHarga.status" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-sm focus:ring-2 focus:ring-indigo-500 transition-all">
    ...
    <x-admin.modal name="forbidden-delete-modal" title="Tidak Dapat Dihapus" icon="alert-triangle">
        <div class="text-center py-4">
            ...
            <p class="text-sm text-slate-600 dark:text-slate-400 leading-relaxed mb-6">
                Kategori tiket ini tidak dapat dihapus karena sudah memiliki riwayat **transaksi yang berhasil**. Menghapus data ini akan merusak validitas laporan keuangan Anda.
            </p>
            <p class="text-xs text-slate-400 bg-slate-50 dark:bg-slate-700/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                Saran: Jika tiket sudah tidak ingin dijual, silakan ubah status menjadi **Nonaktif** melalui tombol toggle di tabel.
            </p>
            <div class="mt-8">
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex justify-between items-start border-b border-slate-100 dark:border-slate-700 pb-4">
                        <div>
                            ...
                            <p class="text-lg font-mono font-bold text-slate-800 dark:text-white">{{ $selectedTransaction->invoice }}</p>
                        </div>
                    ...
                    <div class="flex items-center gap-4 bg-slate-50 dark:bg-slate-700/50 p-4 rounded-2xl">
                        ...
                        <div>
                            <p class="text-sm font-bold text-slate-800 dark:text-white">{{ $selectedTransaction->users->name ?? 'Guest' }}</p>
                        ...
                        <div class="space-y-2">
                            @foreach($selectedTransaction->hargaCarts as $item)
                                <div class="flex justify-between items-center p-3 rounded-xl border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800">
                                    <div>
                                        <p class="text-sm font-bold text-slate-800 dark:text-white">{{ $item->masterHarga->kategori ?? 'Kategori Dihapus' }}</p>
                                        <p class="text-xs text-slate-500">{{ $item->quantity }} x Rp {{ number_format($item->harga_ticket, 0, ',', '.') }}</p>
                                    </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 dark:border-slate-700 space-y-2">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-bold text-slate-800 dark:text-white uppercase">{{ $selectedTransaction->payment_type }}</span>
                        </div>
                        <div class="flex justify-between text-lg font-extrabold pt-2">
                            <span class="text-slate-800 dark:text-white">Total Bayar</span>
                            <span class="text-indigo-600">Rp {{ number_format($selectedTransaction->hargaCarts->sum(fn($i) => $i->quantity * $i->harga_ticket), 0, ',', '.') }}</span>
                        </div>
                    </div>
