@php
    $dialogId = 'interactive-tour-' . $this->getId();
...
>
    <template x-teleport="body">
        <div x-show="isOpen" x-cloak style="display: none;">
            <div
                class="pointer-events-none fixed z-[190] rounded-2xl ring-2 ring-indigo-400 transition-[top,left,width,height] duration-150"
                :style="spotlightStyle"
                aria-hidden="true"
            ></div>

            <section
                class="fixed z-[191] w-[calc(100vw-1.5rem)] max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl dark:border-slate-700 dark:bg-slate-800"
            ...
            >
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <h2 id="{{ $titleId }}" class="mt-1 text-lg font-bold text-slate-900 dark:text-white" x-text="currentStep?.title"></h2>
                    </div>
                ...
                <p id="{{ $descriptionId }}" class="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300" x-text="currentStep?.description"></p>
