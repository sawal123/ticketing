    ...
    <div
        ...
        <div class="relative">
            ...
            <p class="text-slate-400 text-center text-sm mb-10">
                {{ $showOtpStep
                    ? 'Masukkan kode OTP yang telah dikirim ke email Anda untuk menyelesaikan registrasi.'
                    : 'Bergabunglah dengan Gotik dan mulai pengalaman seru.' }}
            </p>
            ...
            @else
                <div class="rounded-2xl border border-white/5 bg-[#1e1d35] px-5 py-4 mb-6">
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Email Tujuan OTP</p>
                    <p class="text-sm text-white font-semibold">{{ $email }}</p>
                    <p class="mt-2 text-xs text-slate-400">Kode OTP berlaku selama 5 menit dan maksimal 5 percobaan.</p>
                </div>

                <form wire:submit.prevent="verifyOtp" class="space-y-6" x-data="{ cooldown: {{ $this->resendCooldown }} }" x-init="setInterval(() => { if (cooldown > 0) cooldown--; }, 1000)">
                    <div>
                        <label for="otp"
                            class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 ml-1">Kode OTP</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none" wire:ignore>
                                <i data-lucide="shield-check" class="h-4 w-4 text-slate-500"></i>
                            </div>
                            <input wire:model.live="otp" id="otp" type="text" inputmode="numeric" maxlength="6" required
                                class="block w-full pl-11 pr-4 py-4 bg-[#1e1d35] border-0 ring-1 ring-white/5 focus:ring-2 focus:ring-indigo-500 rounded-2xl text-white text-sm tracking-[0.35em] placeholder-slate-500 transition-all duration-200"
                                placeholder="000000">
                        </div>
                        @error('otp') <p class="mt-2 text-xs text-rose-500 ml-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="space-y-3">
                        <button type="submit" wire:loading.attr="disabled" wire:target="verifyOtp"
                            ...
                            <span wire:loading.flex wire:target="verifyOtp" class="items-center gap-2" x-cloak>
                                <i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i>
                                Memverifikasi...
                            </span>
                        </button>
                        ...
                        <button type="button" wire:click="resendOtp" wire:loading.attr="disabled" wire:target="resendOtp"
                            :disabled="cooldown > 0"
                            class="w-full flex justify-center py-4 px-4 rounded-2xl bg-white/5 border border-white/5 text-white text-sm font-bold hover:bg-white/10 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed">
                            <span wire:loading.remove wire:target="resendOtp" x-show="cooldown <= 0">Kirim Ulang OTP</span>
                            <span wire:loading.remove wire:target="resendOtp" x-show="cooldown > 0" x-text="`Kirim Ulang OTP (${cooldown}s)`"></span>
                            <span wire:loading.flex wire:target="resendOtp" class="items-center gap-2" x-cloak>
                                <i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i>
                                Mengirim ulang...
                            </span>
                        </button>

                        <button type="button" wire:click="editRegistration"
                            class="w-full flex justify-center py-4 px-4 rounded-2xl bg-transparent border border-white/10 text-slate-300 text-sm font-bold hover:bg-white/5 transition-all duration-200">
                            Kembali / Edit Email
                        </button>
                    </div>
                </form>
