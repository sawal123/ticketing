<aside id="sidebar"
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1" aria-label="Menu utama" x-data="{ 
        ...
        <a href="{{ route('dashboard') }}" wire:navigate
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 {{ request()->routeIs('dashboard') ? 'active' : '' }}">
            <i data-lucide="layout-dashboard" class="w-5 h-5 flex-shrink-0"></i>
            <span class="text-sm font-medium">Dashboard</span>
        </a>

        <!-- b. Event -->
        <a href="{{ auth()->user()->role === 'admin' ? route('admin.event') : route('dashboard.event') }}"
            wire:navigate
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 {{ request()->routeIs('admin.event') || request()->routeIs('dashboard.event') ? 'active' : '' }}">
            <i data-lucide="calendar" class="w-5 h-5 flex-shrink-0"></i>
            <span class="text-sm font-medium">Event</span>
        </a>

        <!-- c. Voucher -->
        <a href="{{ route('dashboard.voucher') }}" wire:navigate
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 {{ request()->routeIs('dashboard.voucher') ? 'active' : '' }}">
            <i data-lucide="ticket" class="w-5 h-5 flex-shrink-0"></i>
            <span class="text-sm font-medium">Voucher</span>
        </a>

        <!-- d. Partner -->
        <a href="{{ route('dashboard.partner') }}" wire:navigate
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 {{ request()->routeIs('dashboard.partner') ? 'active' : '' }}">
        ...
        <!-- f. Penarikan Saldo -->
        <a href="{{ auth()->user()->role === 'admin' ? route('admin.penarikan') : route('dashboard.penarikan') }}"
            wire:navigate
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 {{ request()->routeIs('admin.penarikan') || request()->routeIs('dashboard.penarikan') ? 'active' : '' }}">
            <i data-lucide="wallet" class="w-5 h-5 flex-shrink-0"></i>
            <span class="text-sm font-medium">Penarikan Saldo</span>
        </a>

        <!-- g. Staff -->
        <a href="{{ route('dashboard.staff') }}" wire:navigate
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 {{ request()->routeIs('dashboard.staff') ? 'active' : '' }}">
        ...
        <a href="https://wa.me/6282274677715" target="_blank"
            class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200">
            <i data-lucide="help-circle" class="w-5 h-5 flex-shrink-0 text-emerald-500"></i>
                           ...
                           class="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-all duration-200 group">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0 ring-2 ring-slate-100 dark:ring-slate-800 group-hover:ring-indigo-100 dark:group-hover:ring-indigo-900/50 transition-all">
                                    <img src="{{ asset('storage/cover/' . $event->cover) }}" class="w-full h-full object-cover" alt="{{ $event->event }}">
                                </div>
                                <div class="text-left">
                                    <h4 class="text-sm font-bold text-slate-800 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{{ $event->event }}</h4>
