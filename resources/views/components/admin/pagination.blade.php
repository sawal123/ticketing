@if ($paginator->hasPages())
    <div
        class="flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
        ...
        <nav class="flex items-center gap-1" aria-label="Pagination">
            ...
            @if ($paginator->onFirstPage())
                <button class="cursor-pointer p-2 rounded-lg text-slate-300 dark:text-slate-600 cursor-not-allowed" disabled>
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <button class="px-3 py-1 text-sm cursor-pointer font-bold bg-indigo-600 text-white rounded-lg shadow-sm">
                                {{ $page }}
                            </button>
                        @else
