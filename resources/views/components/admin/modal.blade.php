...
@if ($clientLoading || $viewportScroll)
<div
    x-data="{
        show: false,
        loading: false,
...
>
    <template x-teleport="body">
        <div 
            ...
            class="fixed inset-0 z-[100] flex {{ $viewportScroll ? 'items-start sm:items-center p-2 sm:p-4 overflow-hidden' : 'items-center p-4' }} justify-center bg-slate-900/60 backdrop-blur-sm transition-all duration-300"
            style="display: none;"
        ...
        >
            ...
            <!-- Modal Card -->
            <div 
                class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-{{ $maxWidth }} p-6 relative z-10 border border-slate-200 dark:border-slate-700 {{ $viewportScroll ? 'overflow-y-auto overscroll-contain' : '' }}"
                @if ($viewportScroll)
    ...
    <template x-teleport="body">
        ...
        >
            ...
            <div
                ...
                x-transition:enter-end="opacity-100 scale-100 translate-y-0"
            >
                <button x-on:click="show = false" type="button" aria-label="Tutup dialog" title="Tutup dialog" class="absolute top-4 right-4 p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-400 transition-colors">
                    <i data-lucide="x" class="w-5 h-5"></i>
                </button>
