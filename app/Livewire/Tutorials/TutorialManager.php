...
class TutorialManager extends Component {
    ...
    public function mount(): void { abort_unless(auth()->user()?->role === 'penyewa', 403); }
    public function resetTutorial(string $tutorialKey): void { $user=auth()->user(); if(!$user || $user->role !== 'penyewa' || !isset(self::TUTORIALS[$tutorialKey])) return; app(TutorialProgressService::class)->reset($user,$tutorialKey); if($tutorialKey==='dashboard.getting-started') $this->redirectRoute('dashboard',navigate:true); }
    public function render() { $user=auth()->user(); $progress=app(TutorialProgressService::class); $tutorials=collect(self::TUTORIALS)->map(function($tutorial,$key)use($user,$progress){$tutorial['key']=$key;$tutorial['status']=$progress->isCompleted($user,$key)?'Selesai':($progress->isDismissed($user,$key)?'Dilewati':'Belum selesai');return $tutorial;}); return view('livewire.tutorials.tutorial-manager',compact('tutorials')); }
}
