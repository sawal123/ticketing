...
export function setup() {
  if (!cookie) {
    throw new Error('AUTH_COOKIE is required. This smoke-test script must run as an authenticated test user.');
  }
}

export default function () {
  if (!warnedSingleCookie) {
    console.warn('AUTH_COOKIE tunggal aktif: ini smoke-test authenticated flow, bukan simulasi banyak pembeli unik.');
  ...
  if (!eventPageOk) {
    recordRates({ csrf: 0, validation: 0, rateLimited: 0, server: eventPage.status >= 500 ? 1 : 0, unexpected: 1 });
    fail(`Event page unavailable or unexpected status: ${eventPage.status}`);
  }
  ...
  if (!csrf) {
    recordRates({ csrf: 1, validation: 0, rateLimited: 0, server: 0, unexpected: 0 });
    fail('CSRF token not found on event page.');
  }

  if (!eventUid) {
    recordRates({ csrf: 0, validation: 0, rateLimited: 0, server: 0, unexpected: 1 });
    fail('event_uid not found on event page.');
  }

  if (hargaIds.length === 0) {
    recordRates({ csrf: 0, validation: 0, rateLimited: 0, server: 0, unexpected: 1 });
    fail('No harga_id found on event page. Ensure the staging event has active tickets.');
  }
...
function classifyCheckoutResponse(response, headers) {
  ...
  const accepted = [302, 303].includes(response.status) && location.includes('/detail-ticket/');
...
function isSoldOutRedirect(location, headers) {
  ...
  const redirectUrl = location.startsWith('http') ? location : `${baseUrl}${location.startsWith('/') ? '' : '/'}${location}`;
  const redirected = http.get(redirectUrl, { headers });
  const body = String(redirected.body || '').toLowerCase();

  return redirected.status === 200
    && (
